namespace SeahawkSaverBackend.API.IntegrationTest.Endpoints.Debt.Queries.List;
...
[TestFixture]
public sealed class ListDebtEndpointTest : EndpointTest
{
	[Test]
	public async Task GivenNoBearerToken_WhenAuthenticate_ThenReturnsUnauthorizedStatus()
	{
		var response = await GetAsync(ListDebtEndpointTest.BuildUrl(Guid.Parse("1567C912-FB83-4FF4-91B4-2232807837DB")), null);
	...
	public async Task GivenUserIdInRouteThatDoesNotMatchAuthenticatedUser_WhenAuthenticate_ThenReturnsUnauthorizedStatus()
	{
		...
		var response = await GetAsync(ListDebtEndpointTest.BuildUrl(Guid.Parse("E1E0B144-1DFF-4326-A4E1-6282A58D269B")), token);

		Assert.That(response.StatusCode, Is.EqualTo(HttpStatusCode.Unauthorized));
	}

	[Test]
	public async Task GivenAuthenticatedUserId_WhenListDebt_ThenReturnsOkStatus()
	{
		await SeedDatabaseAsync();
		var token = await GetAuthenticationTokenAsync("vicky.decker@yahoo.com", "#Password4Vicky");
		var response = await GetAsync(ListDebtEndpointTest.BuildUrl(Guid.Parse("1567C912-FB83-4FF4-91B4-2232807837DB")), token);

		Assert.That(response.StatusCode, Is.EqualTo(HttpStatusCode.OK));
	}

	[Test]
	public async Task GivenAuthenticatedUser_WhenListDebt_ThenReturnsDebtForUser()
	{
		await SeedDatabaseAsync();
		var token = await GetAuthenticationTokenAsync("vicky.decker@yahoo.com", "#Password4Vicky");
		var response = await GetAsync(ListDebtEndpointTest.BuildUrl(Guid.Parse("1567C912-FB83-4FF4-91B4-2232807837DB")), token);
		...
		Assert.That(content.Debts, Has.Count.EqualTo(2));
	}
	...
	{
		return $"{DebtEndpointsMapper.Prefix}/list/{userId}";
	}
}