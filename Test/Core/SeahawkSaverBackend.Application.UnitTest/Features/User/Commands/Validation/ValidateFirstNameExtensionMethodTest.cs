...
[TestFixture]
public sealed class ValidateFirstNameExtensionMethodTest : ExtensionMethodValidationTest<FakeUserCommandRequest>
{
	...
	{
		base.SetUp();
		Validator.RuleFor(user => user.FirstName)
				 .ValidateFirstName();
	}
	...
	public static IEnumerable<ExtensionMethodValidationTestCase<FakeUserCommandRequest>> InvalidDTOs
	{
		get
		{
			yield return new ExtensionMethodValidationTestCase<FakeUserCommandRequest>
			{
				...
				ExpectedErrorPropertyName = nameof(FakeUserCommandRequest.FirstName)
			};
		}
	}
