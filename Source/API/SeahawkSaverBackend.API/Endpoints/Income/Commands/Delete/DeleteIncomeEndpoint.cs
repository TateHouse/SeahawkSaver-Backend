namespace SeahawkSaverBackend.API.Endpoints.Income.Commands.Delete;
using MediatR;
...
/**
 * <summary>
 * An endpoint for deleting an existing <see cref="SeahawkSaverBackend.Domain.Entities.Income"/> entity.
 * </summary>
 */
public static class DeleteIncomeEndpoint
{
	...
	public static void MapEndpoint(RouteGroupBuilder groupBuilder, string[] tags)
	{
		groupBuilder.MapDelete("/{userId}", DeleteIncomeEndpoint.HandleAsync)
					...
					.WithDescription("For a user to delete an existing income from the database, he must provide his user id and the income id.")
	...
	private async static Task<IResult> HandleAsync(IMediator mediator,
												   ...
												   [FromQuery] Guid incomeId)
	{
		try
		{
			var commandSettings = new CommandSettings(true, true);
			var command = DeleteIncomeCommandFactory.Create(commandSettings, incomeId, userId);
